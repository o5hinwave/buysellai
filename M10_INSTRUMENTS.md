...
Metadata is validated too: `Date` must use `YYYY-MM-DD`, `Device model` must mention iPhone or iPad, `iOS version` and `Release build` must include numeric values, and `Signed archive` must mention an archive. Before final submit-readiness can pass, `Device model` must also include the real-device preflight `device name:` value, `Signed archive` must also include the signed-preflight `archive:` path, and `Release build` must match the signed-preflight `release build:` marker. The `Time Profiler trace` and `Allocations trace` metadata values must point to retained trace files or directories. Relative paths are resolved from this evidence file's directory.
