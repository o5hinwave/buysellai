...
final class RealDevicePreflightScriptTests: XCTestCase {
    func testRealDevicePreflightDiscoversConnectedDeviceWithDevicectlJSON() throws {
        ...
        XCTAssertNotNil(script.range(of: #"entitlements_path="${work_root}/BuySellAI/BuySellAI.entitlements""#))
        XCTAssertNotNil(script.range(of: "no connected iPhone or iPad"))
        XCTAssertNotNil(script.range(of: "snapshot root:"))
        XCTAssertNotNil(script.range(of: "print_source_release_markers"))
        XCTAssertNotNil(script.range(of: "source_bundle_identifier"))
        XCTAssertNotNil(script.range(of: "source_sign_in_with_apple"))
        XCTAssertNotNil(script.range(of: "source_project_setting MARKETING_VERSION"))
        XCTAssertNotNil(script.range(of: "source_project_setting CURRENT_PROJECT_VERSION"))
    ...
    func testAcceptanceDocsRouteRealDeviceAcceptanceThroughPreflight() throws {
        ...
        XCTAssertNotNil(readme.range(of: "M10_REAL_DEVICE_SNAPSHOT_ROOT=/tmp/buysell-m10-real-device-worktree"))
        XCTAssertNotNil(m10.range(of: "M10_REAL_DEVICE_SNAPSHOT_ROOT=/tmp/buysell-m10-real-device-worktree"))
        XCTAssertNotNil(readme.range(of: "M10_DEVELOPMENT_TEAM=<team-id>"))
        XCTAssertNotNil(m10.range(of: "M10_DEVELOPMENT_TEAM=<team-id>"))
        XCTAssertNotNil(readme.range(of: "/tmp/buysell-submit-readiness-real-device-preflight.log"))
        XCTAssertNotNil(m10.range(of: "/tmp/buysell-submit-readiness-real-device-preflight.log"))
        ...
        XCTAssertNotNil(readme.range(of: "Passing logs include `device:`, `device name:`, `device id:`, optional `snapshot root:`, `app:`, `bundle id:`, `sign in with apple:`, and `release build:` markers"))
        XCTAssertNotNil(readme.range(of: "Allowed no-device pending logs still include source-level `bundle id:`, `sign in with apple:`, and `release build:` markers"))
        XCTAssertNotNil(m10.range(of: "A supplied `DEVICE_ID` must match a connected iPhone or iPad"))
        XCTAssertNotNil(m10.range(of: "Allowed no-device pending logs still retain source-level `bundle id:`, `sign in with apple:`, and `release build:` markers"))
    }
    ...
    func testRealDeviceAcceptanceEvidenceScriptRequiresMetadataAndPromptRows() throws {
        ...
        XCTAssertNotNil(script.range(of: "is_generic_evidence"))
        XCTAssertNotNil(script.range(of: "evidence is too generic; cite observed proof instead of a pass note"))
        ...
        XCTAssertNotNil(script.range(of: "Signed archive validation"))
        XCTAssertNotNil(script.range(of: "Xcode Organizer validation proof"))
        XCTAssertNotNil(script.range(of: "archive validation proof"))
        ...
        XCTAssertNotNil(script.range(of: "marketplace list, payouts, Best, Lowest, and all 27 prompt platforms"))
        XCTAssertNotNil(script.range(of: "required_marketplace_terms"))
        XCTAssertNotNil(script.range(of: "all 27 prompt platforms"))
        XCTAssertNotNil(script.range(of: "\"ruby lane\""))
        XCTAssertNotNil(script.range(of: "\"tcgplayer\""))
        XCTAssertNotNil(script.range(of: "clipboard listing text with no leading whitespace or preamble proof"))
        XCTAssertNotNil(script.range(of: "guest SwiftData and signed-in server relaunch persistence proof"))
        XCTAssertNotNil(script.range(of: "Settings Reduce Motion app-wide animation proof"))
    ...
    func testRealDeviceAcceptanceDocsRouteManualEvidenceThroughScript() throws {
        ...
        XCTAssertNotNil(m10.range(of: "`Signed archive validation` must mention Xcode Organizer validation proof"))
        ...
        XCTAssertNotNil(m10.range(of: "A05 `platform`, `payout`, `best`, `lowest`, `27`, `ebay`"))
        XCTAssertNotNil(m10.range(of: "`ruby lane`, `tcgplayer`"))
        XCTAssertNotNil(m10.range(of: "A06 `clipboard`, `listing text`, `no leading whitespace`, `no preamble`"))
        XCTAssertNotNil(m10.range(of: "A09 `relaunch`, `guest`, `swiftdata`, `signed`, `server`"))
        XCTAssertNotNil(m10.range(of: "A10 `settings`, `reduce motion`, `animation`, `app-wide`"))
    ...
    func testRealDeviceAcceptanceDocsStartPendingUntilDeviceEvidenceIsRecorded() throws {
        ...
        XCTAssertNotNil(m10.range(of: "| Signed archive validation | TBD |"))
