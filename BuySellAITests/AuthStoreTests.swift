...
final class AuthStoreTests: XCTestCase {
    ...
    func testAuthViewDisablesEntryPointsDuringSignIn() throws {
        ...
        XCTAssertNotNil(source.range(of: #"Label\("Continue with Apple"\.localized, systemImage: "apple\.logo"\)[\s\S]*?\.disabled\(store\.isSigningIn\)"#, options: .regularExpression))
        XCTAssertNotNil(source.range(of: #"NavigationLink\(value: AuthRoute\.email\)[\s\S]*?\.disabled\(store\.isSigningIn\)"#, options: .regularExpression))
        XCTAssertNotNil(source.range(of: #"Text\("Keep going without an account"\.localized\)[\s\S]*?\.disabled\(store\.isSigningIn\)"#, options: .regularExpression))
    ...
    func testAuthLandingKeepsGuestEscapeStickyAndDynamicTypeAware() throws {
        ...
        XCTAssertNotNil(source.range(of: #".contentMargins(.bottom, authBottomContentInset, for: .scrollContent)"#))
        XCTAssertNotNil(source.range(of: #"Text("Keep going without an account".localized)"#))
        XCTAssertNotNil(source.range(of: #".buttonStyle(.bordered)"#))
        XCTAssertNotNil(source.range(of: #".background(.bar)"#))
