...
final class SigningCapabilityTests: XCTestCase {
    ...
    func testSignedSubmitGatesStayDocumentedWhileDevelopmentTeamIsUnset() throws {
        ...
        XCTAssertNotNil(readme.range(of: "A signed archive, App Store Connect IPA export, App Store Connect validation, App Store metadata pass, backend smoke preflight, and Instruments pass are blocked until an Apple development team, App Store Connect API-key credentials, completed product-page metadata, real Supabase config, and trusted physical hardware are available"))
