...
final class DesignAccessibilityTests: XCTestCase {
    ...
    func testSnapResultRateLimitUsesCalmCooldownStateWithoutRetryLoop() throws {
        let source = try String(contentsOf: projectURL("BuySellAI/Features/SnapResult/SnapResultSheet.swift"), encoding: .utf8)
        let cooldownRange = try XCTUnwrap(source.range(of: "private func protectionCooldownView(message: String) -> some View"))
        let retakeRange = try XCTUnwrap(source.range(of: "private var errorRetakeButton: some View", range: cooldownRange.upperBound..<source.endIndex))
        let cooldownSource = String(source[cooldownRange.lowerBound..<retakeRange.lowerBound])

        XCTAssertNotNil(source.range(of: #"if isAnalysisProtectionCooldown(message) {"#))
        XCTAssertNotNil(source.range(of: #"message == APIError.rateLimited.localizedDescription"#))
        XCTAssertNotNil(cooldownSource.range(of: #"Image(systemName: "clock.badge.checkmark")"#))
        XCTAssertNotNil(cooldownSource.range(of: #"Text("Short break".localized)"#))
        XCTAssertNotNil(cooldownSource.range(of: #"Text(message)"#))
        XCTAssertNotNil(cooldownSource.range(of: #"viewSavedListingsButton"#))
        XCTAssertNotNil(cooldownSource.range(of: #"errorRetakeButton"#))
        XCTAssertNotNil(source.range(of: #"Label("View saved listings".localized, systemImage: "list.bullet.rectangle")"#))
        XCTAssertNotNil(source.range(of: #"appStore.closeFlow()"#))
        XCTAssertNotNil(source.range(of: #".accessibilityHint("Closes this scan and keeps your saved listings available.".localized)"#))
        XCTAssertNil(cooldownSource.range(of: #"Color.brand.destructive"#))
        XCTAssertNil(cooldownSource.range(of: #"Try a clearer photo, or retry this one."#))
        XCTAssertNil(cooldownSource.range(of: #"errorRetryButton"#))
        XCTAssertNil(cooldownSource.range(of: #".task(id: message)"#))
    }
...
"""#))
        ...
        XCTAssertNotNil(source.range(of: #"compactQuestionImpactLine(for: question)"#))
        XCTAssertNotNil(source.range(of: #"private func compactQuestionImpactLine(for question: DetailQuestion) -> some View"#))
        XCTAssertNotNil(source.range(of: #"ItemQuestions.CompactImpactLine"#))
        XCTAssertNotNil(source.range(of: #"private static func compactQuestionImpactText("#))
        XCTAssertNotNil(source.range(of: #"Narrows what BuySell searches next."#))
        XCTAssertNotNil(source.range(of: #"Checks if this is a version worth more."#))
        XCTAssertNotNil(source.range(of: #"Helps compare the right sold prices."#))
        XCTAssertNotNil(source.range(of: #"Helps the %@ post use the right fields."#))
        XCTAssertNotNil(source.range(of: #"Keeps the listing honest without guessing."#))
        XCTAssertNotNil(source.range(of: #"One clue is enough. Skip if you do not know."#))
...
"""#))
    ...
    func testItemQuestionsSheetCoversBroadMarketplaceSpecificPrompts() throws {
        ...
        [
            ...
            #"let warningSeed: (warning: String, field: Field)?"#,
    ...
    func testListingExposesNativeCopyButtonsForUsefulDraftFields() throws {
        ...
        XCTAssertNotNil(listing.range(of: #"private var copyPiecesDisclosure: some View"#))
        XCTAssertNotNil(listing.range(of: #"Label("Copy pieces".localized, systemImage: "doc.on.doc")"#))
        XCTAssertNotNil(listing.range(of: #".accessibilityIdentifier("Listing.CopyPiecesDisclosure")"#))
        ...
        XCTAssertNotNil(listing.range(of: #"title: pricePlan.hasVerifiedMarketPricing ? "Price" : "Starting price estimate""#))
        ...
        XCTAssertNotNil(listing.range(of: #"negotiationFloor.currency(code: context.item.currencyCode)"#))
    ...
    func testListingStructuredDraftGuidanceUsesNativeTipRows() throws {
        let listing = try String(contentsOf: projectURL("BuySellAI/Features/Listing/ListingSheet.swift"), encoding: .utf8)
        let photosRange = try XCTUnwrap(listing.range(of: #"private var photosDisclosure: some View"#))
        let pricePlanRange = try XCTUnwrap(listing.range(of: #"private var pricePlanDisclosure: some View"#, range: photosRange.upperBound..<listing.endIndex))
        let photosSource = String(listing[photosRange.lowerBound..<pricePlanRange.lowerBound])
        let quickTipsRange = try XCTUnwrap(listing.range(of: #"private var quickTipsDisclosure: some View"#))
        let listingNextStepsRange = try XCTUnwrap(listing.range(of: #"private var listingNextSteps: [ListingNextStep]"#, range: quickTipsRange.upperBound..<listing.endIndex))
        let quickTipsSource = String(listing[quickTipsRange.lowerBound..<listingNextStepsRange.lowerBound])
        ...
        XCTAssertNotNil(photosSource.range(of: #"Label("Photos".localized, systemImage: "photo.on.rectangle")"#))
        XCTAssertNotNil(photosSource.range(of: #".accessibilityIdentifier("Listing.PhotosDisclosure")"#))
        ...
        XCTAssertNotNil(listing.range(of: #"private var postingChecklistDisclosure: some View"#))
        XCTAssertNotNil(listing.range(of: #"Label("Check before posting".localized, systemImage: "exclamationmark.triangle.fill")"#))
        XCTAssertNotNil(listing.range(of: #".accessibilityIdentifier("Listing.PostingChecklistDisclosure")"#))
        XCTAssertNotNil(listing.range(of: #"title: "Missing details""#))
        ...
        XCTAssertNotNil(listing.range(of: #"private var pricePlanDisclosure: some View"#))
        XCTAssertNotNil(listing.range(of: #"Label("Price plan".localized, systemImage: "dollarsign.circle.fill")"#))
        XCTAssertNotNil(listing.range(of: #".accessibilityIdentifier("Listing.PricePlanDisclosure")"#))
        XCTAssertNotNil(listing.range(of: #"private var soldCompsDisclosure: some View"#))
        XCTAssertNotNil(listing.range(of: #"Label("What it sells for".localized, systemImage: "chart.line.uptrend.xyaxis")"#))
        XCTAssertNotNil(listing.range(of: #".accessibilityIdentifier("Listing.SoldCompsDisclosure")"#))
        XCTAssertNotNil(listing.range(of: #"Label("Quick tips".localized, systemImage: "lightbulb.fill")"#))
        XCTAssertNotNil(listing.range(of: #".accessibilityIdentifier("Listing.QuickTipsDisclosure")"#))
    ...
    func testListingLeadsWithGeneratedTextBeforeMarketplaceDetails() throws {
        ...
        let recommendationRange = try XCTUnwrap(listing.range(of: "listingRecommendationSummary", range: nextStepsRange.upperBound..<listing.endIndex))
        let detailsRange = try XCTUnwrap(listing.range(of: #"Section("Details".localized) {"#, range: recommendationRange.upperBound..<listing.endIndex))
        let copyPiecesRange = try XCTUnwrap(listing.range(of: "copyPiecesDisclosure", range: detailsRange.upperBound..<listing.endIndex))
        let priceRange = try XCTUnwrap(listing.range(of: "pricePlanDisclosure", range: copyPiecesRange.upperBound..<listing.endIndex))
        ...
        XCTAssertLessThan(nextStepsRange.lowerBound, recommendationRange.lowerBound)
        XCTAssertLessThan(recommendationRange.lowerBound, detailsRange.lowerBound)
        XCTAssertLessThan(detailsRange.lowerBound, copyPiecesRange.lowerBound)
        XCTAssertLessThan(recommendationRange.lowerBound, priceRange.lowerBound)
        XCTAssertNil(listing.range(of: #"Section("Photos".localized) {"#))
        XCTAssertNil(listing.range(of: #"Section("Price plan".localized) {"#))
        XCTAssertNil(listing.range(of: #"Section("Quick tips".localized) {"#))
        ...
        XCTAssertNotNil(listing.range(of: #"takeHomeEstimate.currency(code: context.item.currencyCode)"#))
        XCTAssertNotNil(listing.range(of: #"Text("Current take-home could not be verified.".localized)"#))
        XCTAssertNotNil(listing.range(of: #"title: "Pricing not verified""#))
        XCTAssertNotNil(listing.range(of: #"BuySell did not find reliable sold-price evidence, so it is not guessing a sale price, take-home amount, or lowest offer."#))
        XCTAssertNotNil(listing.range(of: #"title: "Lowest to take""#))
        XCTAssertNotNil(listing.range(of: #"value: negotiationFloor"#))
        ...
        XCTAssertNotNil(listing.range(of: #".accessibilityValue(recommendationPriceAccessibilityValue)"#))
    }
    ...
    func testListingSuccessActionsAreStickyAndOrderedAfterGenerationSucceeds() throws {
        ...
        XCTAssertNotNil(listing.range(of: #"Label("Save cover photo".localized, systemImage: "photo")"#))
        XCTAssertNotNil(listing.range(of: #"Label("Share cover photo".localized, systemImage: "square.and.arrow.up")"#))
        XCTAssertNotNil(listing.range(of: #".accessibilityLabel("Photo actions".localized)"#))
        XCTAssertNotNil(listing.range(of: #".accessibilityHint("Save or share this selected listing photo.".localized)"#))
        ...
        XCTAssertNotNil(listing.range(of: #"private func shareOrExportPhoto(photoID: UUID)"#))
        XCTAssertNotNil(listing.range(of: #"private func savePhotosToLibrary(scope: ListingPhotoExportScope)"#))
        XCTAssertNotNil(listing.range(of: #"private func savePhotoToLibrary(photoID: UUID)"#))
        XCTAssertNotNil(listing.range(of: #"ListingPhotoLibrarySaver.save(exports)"#))
        XCTAssertNotNil(listing.range(of: #"ListingPhotoLibrarySaver.save([export])"#))
        ...
        XCTAssertNotNil(listing.range(of: #""export_type": "photo_single""#))
        XCTAssertNotNil(listing.range(of: #""export_type": scope == .recommended ? "apple_photos" : "apple_photos_all""#))
        XCTAssertNotNil(listing.range(of: #""export_type": "apple_photos_single""#))
        XCTAssertNotNil(listing.range(of: #""photo_scope": scope.rawValue"#))
        XCTAssertNotNil(listing.range(of: #""photo_scope": "single""#))
        XCTAssertNotNil(listing.range(of: #""photo_role": export.role.rawValue"#))
        ...
        XCTAssertNotNil(listing.range(of: #"photoPackage.exportFile(for: context.item, photoID: photoID)"#))
