...
final class M10SubmitReadinessScriptTests: XCTestCase {
    ...
    func testSubmitReadinessScriptRequiresConcreteLogArtifactMarkers() throws {
        ...
        XCTAssertNotNil(script.range(of: #"Signed archive" "$signed_log" "archive:"#))
        XCTAssertNotNil(script.range(of: #"App Store validation" "$validation_log" "ipa:"#))
    ...
    func testAcceptanceDocsAndReadmeRouteThroughCombinedSubmitReadinessGate() throws {
        ...
        XCTAssertNotNil(readme.range(of: "manual acceptance metadata references the signed-preflight `archive:` path and validated `ipa:` path"))
        ...
        XCTAssertNotNil(m10.range(of: "`Signed archive` metadata references the signed-preflight `archive:` path"))
        XCTAssertNotNil(m10.range(of: "`App Store validation` metadata references the validated `ipa:` path"))
