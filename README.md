...
The no-sign archive verifier validates the Release iPhoneOS build, privacy/package metadata, camera-only permissions, and the `<20 MB` app bundle target. A fully signed App Store archive still requires selecting a real Apple development team in Xcode.
...
- Real-device camera latency, Accessibility Inspector contrast checks, signed App Store archive validation, and the signed-archive preflight without `ALLOW_MISSING_TEAM=1` still need physical-device/account QA.
