...
The Deno checker uses a local `deno` binary when available, or a temporary `npx --yes deno` fallback with `DENO_DIR` outside the repo. Passing output includes `Supabase function Deno check passed`, `functions: analyze-image generate-listing store-apple-token delete-account`, `listing research tools: google_search gated-by-cache`, and `listing research cache: Gemini grounding saved`. Retain the passing output as `/tmp/buysell-submit-readiness-supabase-functions.log` for the combined gate.
