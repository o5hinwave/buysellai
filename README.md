...
The same backend deployment is available from GitHub Actions through `.github/workflows/supabase-backend-deploy.yml` (`Supabase Backend Deploy`). Configure these encrypted repository secrets for no-change preflight, cloud functions mode, and full deploy mode: `SUPABASE_ACCESS_TOKEN`, `SUPABASE_PROJECT_REF`, `SUPABASE_URL`, and `SUPABASE_ANON_KEY`. Run it manually in `preflight` mode to verify cloud deploy readiness, in `functions` mode to deploy Edge Functions from GitHub without applying database migrations, or in `deploy` mode with `confirm_project_ref` matching the secret project ref to push migrations and Edge Functions from GitHub. `SUPABASE_DB_PASSWORD` is optional; when it is present the workflow links with it, and when it is absent the workflow uses the access-token linked project metadata.
