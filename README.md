...
Use the `codex/m10-submit-readiness` branch for active agent work until it is reviewed and merged into `main`. The repository includes `.github/workflows/ios-ci.yml`, which runs a hosted macOS iOS check on pushes and pull requests so the app can be worked on away from this Mac. The broader `.github/workflows/cloud-handoff.yml` workflow is named `Cloud Handoff Check`; it can also be launched manually from GitHub Actions and verifies the portable checkout, secret scan, Supabase schema/function sources, support-site build/test, and a native iOS smoke bundle. That hosted check is the quick proof that the app, backend templates, App Store support site, and release docs are present in the cloud repo rather than living only on this computer. For App Store release automation, connect this same GitHub repository to Xcode Cloud or a GitHub Actions release workflow, then store signing credentials, App Store Connect API keys, Supabase service-role secrets, Gemini keys, and Apple private-key material only in the cloud provider's encrypted secret store. Do not commit those values.
...
The workflow source is `.github/workflows/cloud-handoff.yml`. It runs on GitHub's hosted macOS runner and checks `Scripts/check_workspace_materialization.sh`, `Scripts/scan_m10_secrets.sh`, `Scripts/check_supabase_schema.sh`, `Scripts/check_supabase_functions.sh`, `AppStoreSite` `npm test`, and a focused native iOS smoke bundle. It does not deploy Supabase, submit to App Store Connect, or expose secrets; those remain behind explicit release credentials and the existing deploy/preflight gates.
