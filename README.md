...
- Real-device camera latency, Accessibility Inspector contrast checks, signed App Store archive validation, the signed-archive preflight without `ALLOW_MISSING_TEAM=1`, and the real-device preflight without `ALLOW_MISSING_DEVICE=1` still need physical-device/account QA.
