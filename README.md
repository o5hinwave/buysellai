...
The same backend deployment is available from GitHub Actions through `.github/workflows/supabase-backend-deploy.yml` (`Supabase Backend Deploy`). Configure these encrypted repository secrets first: `SUPABASE_ACCESS_TOKEN`, `SUPABASE_PROJECT_REF`, `SUPABASE_DB_PASSWORD`, `SUPABASE_URL`, and `SUPABASE_ANON_KEY`. The workflow keeps ordinary push checks non-deploying; run it manually in `preflight` mode to verify cloud deploy readiness, or in `deploy` mode with `confirm_project_ref` matching the secret project ref to push migrations and Edge Functions from GitHub.
