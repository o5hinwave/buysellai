...
require_file_contains "$no_sign_log" "bundle id: com.despia.buysellai" "no-sign archive verifier log"
...
require_file_contains "$no_sign_log" "release fixtures: none bundled" "no-sign archive verifier log"
...
require_file_contains "$latest_design_log" "home setup: pearl Apple Card-style centered camera card, always-visible recent listings empty state, native icon trail, circular listing loop badge, native 1-2-3 task rows, quiet footer copy, restrained warm accent, monochrome toolbar wordmark, account, and settings actions" "latest design SDK evidence log"
...
require_file_contains "$source_typecheck_log" "modes: release app debug app unit ui" "local source typecheck log"
...
require_file_contains "$signed_log" "bundle id: com.despia.buysellai" "signed archive preflight log"
...
require_file_contains "$export_log" "bundle id: com.despia.buysellai" "App Store export preflight log"
...
require_file_contains "$validation_log" "bundle id: com.despia.buysellai" "App Store validation preflight log"
...
require_file_contains "$metadata_log" "bundle id: com.despia.buysellai" "App Store metadata evidence log"
...
require_file_contains "$metadata_log" "iphone 6.9 screenshot result bundle: /tmp/buysell-m10-screenshots-iphone-16-pro-max-apple-card-20260723.xcresult" "App Store metadata evidence log"
require_file_contains "$metadata_log" "ipad 13 screenshot result bundle: /tmp/buysell-m10-screenshots-ipad-pro-13-apple-card-20260723.xcresult" "App Store metadata evidence log"
...
require_file_contains "$backend_log" "schema: history apple_auth_tokens marketplace_research_cache entitlement_config entitlement_usage_events" "backend preflight log"
require_file_contains "$backend_log" "functions: backend-health analyze-image compare-marketplaces generate-listing store-apple-token delete-account" "backend preflight log"
require_file_contains "$backend_log" "protected functions: store-apple-token delete-account" "backend preflight log"
require_file_contains "$backend_log" "protected tables: history apple_auth_tokens marketplace_research_cache entitlement_config entitlement_usage_events" "backend preflight log"
...
require_file_contains "$supabase_deploy_log" "schema: history apple_auth_tokens marketplace_research_cache entitlement_config entitlement_usage_events" "Supabase deploy log"
require_file_contains "$supabase_deploy_log" "constraints: history category condition marketplace listing metadata identification-profile marketplace-comparison supplemental-photos apple-token-identity marketplace-research-cache early-access-entitlements usage-protection" "Supabase deploy log"
require_file_contains "$supabase_deploy_log" "functions: backend-health analyze-image compare-marketplaces generate-listing store-apple-token delete-account" "Supabase deploy log"
...
require_file_contains "$supabase_schema_log" "tables: history apple_auth_tokens marketplace_research_cache entitlement_config entitlement_usage_events" "Supabase schema static check log"
require_file_contains "$supabase_schema_log" "rls: history apple_auth_tokens marketplace_research_cache entitlement_config entitlement_usage_events forced" "Supabase schema static check log"
...
require_file_contains "$supabase_schema_log" "grants: history authenticated service_role apple_auth_tokens service_role marketplace_research_cache service_role entitlement_config service_role entitlement_usage_events service_role" "Supabase schema static check log"
require_file_contains "$supabase_schema_log" "constraints: history category condition marketplace listing metadata identification-profile marketplace-comparison supplemental-photos apple-token-identity marketplace-research-cache early-access-entitlements usage-protection" "Supabase schema static check log"
...
require_file_contains "$supabase_function_log" "functions: backend-health analyze-image compare-marketplaces generate-listing store-apple-token delete-account" "Supabase function type-check log"
...
require_file_contains "$supabase_function_log" "early access: server-controlled entitlements with usage protection" "Supabase function type-check log"
require_file_contains "$supabase_function_log" "listing evidence sources: sold-comp guarded structured source/date/status/comparability" "Supabase function type-check log"
...
require_file_contains "$real_device_log" "bundle id: com.despia.buysellai" "real-device preflight log"
...
require_file_contains "$ui_evidence_log" "tests: 33" "chunked UI evidence log"
require_file_contains "$ui_evidence_log" "test: BuySellAIUITests/BuySellAIUITests/testListingCanReturnToMarketplacePickerWithoutRetakingPhoto" "chunked UI evidence log"
...
require_file_contains "$performance_log" "bundle id: com.despia.buysellai" "performance evidence log"
