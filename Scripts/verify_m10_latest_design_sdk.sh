...
if (( ${#pending_items[@]} > 0 )); then
    if [[ "$allow_missing_sdk" == "1" ]]; then
        ...
        printf 'home setup: pearl Apple Card-style centered camera card, always-visible recent listings empty state, native icon trail, circular listing loop badge, native 1-2-3 task rows, quiet footer copy, restrained warm accent, monochrome toolbar wordmark, account, and settings actions\n'
