...
GitHub Actions can run the same deploy path from the cloud through `.github/workflows/supabase-backend-deploy.yml` (`Supabase Backend Deploy`). Configure encrypted repository secrets named `SUPABASE_ACCESS_TOKEN`, `SUPABASE_PROJECT_REF`, `SUPABASE_URL`, and `SUPABASE_ANON_KEY`, then run the workflow manually in `preflight` mode for a no-change readiness check, `functions` mode to deploy Edge Functions without database migrations, or `deploy` mode with `confirm_project_ref` matching `SUPABASE_PROJECT_REF` to push migrations and Edge Functions. `SUPABASE_DB_PASSWORD` is optional; the workflow links with it when present and otherwise uses access-token linked project metadata. The workflow runs the secret scan, writes only the public app config, records the project ref, verifies schema/function sources, and calls `Scripts/deploy_supabase_backend.sh`; it does not store provider keys, service-role keys, or Apple private-key material in source.
