...
- Simulator suite: `503` unit/UI tests pass on iPhone 16 Pro simulator, iOS 18.6, from the latest app-source evidence bundle `/tmp/buysell-submit-readiness-full-8806b12-1784510561.xcresult`.
- Unit target: `470` `BuySellAITests` tests pass on iPhone 16 Pro simulator, iOS 18.6, in the retained default unit bundle `/tmp/buysell-full-unit-tests-9.xcresult`.
- Chunked simulator UI evidence: all `32` `BuySellAIUITests` tests pass through `M10_UI_SNAPSHOT_ROOT=/tmp/buysell-m10-ui-worktree-8806b12 bash Scripts/run_m10_ui_tests.sh`, with the rollup retained at `/tmp/buysell-m10-ui-tests/summary.log`, verifier evidence retained at `/tmp/buysell-submit-readiness-ui-8806b12.log`, and per-test `.xcresult`/`.log` artifacts under `/tmp/buysell-m10-ui-tests/`.
- Focused M10 guardrail coverage: the latest app-source full-suite result bundle `/tmp/buysell-submit-readiness-full-8806b12-1784510561.xcresult` includes the required M10 script, metadata, Today nomination, backend, signing, config-security, performance, UI-evidence, localization, camera, and real-device guardrails; the two previously failing localization/Today checks also pass in focused evidence at `/tmp/buysell-localization-today-focused-1784509943.xcresult`.
- No-sign Release iPhoneOS archive: compiles and packages a `2260KB` app bundle from the lean snapshot worktree `/private/tmp/buysell-m10-archive-worktree-8806b12`; refreshed evidence is retained at `/tmp/buysell-submit-readiness-nosign-8806b12.log` and `/tmp/buysell-submit-readiness-nosign-8806b12.xcarchive`.
...
- Font setup polish: Space Grotesk and Inter legacy font assets have been removed from the synchronized app folder, `Info.plist` does not register `UIAppFonts`, every `BrandTextStyle` standard and Bold Text variant resolves to native San Francisco semantic Dynamic Type, and SF Symbol sizing routes through shared `BrandSymbolStyle` tokens.
...
- Workspace materialization check: `Scripts/check_workspace_materialization.sh` records whether Git metadata, app source, tests, Supabase sources, support-site sources, M10 docs, release verifiers, and retained App Store screenshots are locally readable instead of `dataless`. Latest app-source evidence is retained at `/tmp/buysell-submit-readiness-workspace-materialization-8806b12.log`: `M10 workspace materialization passed`, Git can index the worktree, app/unit/UI/Supabase/support-site sources are locally readable, required iPhone 6.9-inch screenshots are `1320x2868`, required iPad 13-inch screenshots are `2064x2752`, and README/M10 acceptance/metadata/Today nomination docs are readable.
- Local source verification: `Scripts/typecheck_local_sources.sh` now passes from the materialized checkout for app, unit-test, and UI-test Swift sources using the iPhone Simulator 18.5 SDK; latest app-source evidence is retained at `/tmp/buysell-submit-readiness-typecheck-8806b12.log`.
- Supabase source verification: `Scripts/check_supabase_schema.sh` passes with `history` and `apple_auth_tokens`, forced RLS, `(select auth.uid())` policy coverage, policy/index parity, history value/listing constraints, and unique Apple token identity storage; latest app-source evidence is retained at `/tmp/buysell-submit-readiness-supabase-schema-8806b12.log`. `Scripts/check_supabase_functions.sh` passes Deno checks for `analyze-image`, `generate-listing`, `store-apple-token`, and `delete-account`; latest app-source evidence is retained at `/tmp/buysell-submit-readiness-supabase-functions-8806b12.log`. `Scripts/setup_supabase_secrets.sh preflight` now verifies Supabase CLI secret access before any secret values are requested, and the `full` and `gemini-only` modes run the same no-secret access check before prompting or reading CI secret variables. Current live Supabase deploy/backend evidence is still blocked because `BuySellAI/App/Config.plist` is missing and real Supabase project access/deploy state is not configured on this machine; no provider key has been written to the repository or app bundle.
- Support-site source verification: `npm run build` in `AppStoreSite/` now clears stale local Wrangler deploy state, rebuilds the Vinext/Sites output, validates fresh `dist/server/index.js`, `dist/client`, and `dist/.openai/hosting.json` output, and renders `/`, `/support`, `/privacy`, `/accessibility`, and `/terms` before printing `BuySell support site build passed`; refreshed build evidence is retained at `/tmp/buysell-support-site-build.log`. `npm test` then rebuilds and passes `3` rendered-page tests for the public support, privacy, accessibility, and terms pages, with evidence retained at `/tmp/buysell-support-site-test.log`.
...
- Simulator performance evidence: launch, camera-ready, capture-to-result thumbnail, slow history load, and 500-row history scroll UI tests pass in the full suite; `Scripts/verify_m10_performance_evidence.sh` passed with 5 performance tests and the refreshed `2260KB / 20480KB` app-size result in `/tmp/buysell-submit-readiness-performance-8806b12.log`.
- App Store screenshot evidence: required iPhone 6.9-inch `1320 x 2868` PNGs are retained in `AppStoreAssets/Screenshots/iPhone-16-Pro-Max/`, required iPad 13-inch `2064 x 2752` PNGs are retained in `AppStoreAssets/Screenshots/iPad-Pro-13-inch-M4/`, both sets pass blank/dark-strip artifact and warm orange brand-signal checks, and both were captured by `BuySellAIUITests/BuySellAIUITests/testM10AppStoreScreenshotsCanBeCaptured` in standalone result bundles `/tmp/buysell-m10-screenshots-iphone-16-pro-max.xcresult` and `/tmp/buysell-m10-screenshots-ipad-pro-13.xcresult`; the broader latest app-source submit-readiness full-suite result bundle `/tmp/buysell-submit-readiness-full-8806b12-1784510561.xcresult` is also retained.
- Apple Today feature nomination evidence: `M10_TODAY_FEATURE_NOMINATION.md` records App Store Connect Featuring Nominations copy, Apple guidance/source links, lead-time expectations, editorial story, technology/accessibility/privacy angles, retained screenshot assets, and the text-free App Store icon; `Scripts/verify_m10_today_feature_nomination.sh` validates the package and writes `/tmp/buysell-submit-readiness-today-feature-8806b12.log`, which the combined submit-readiness gate now requires.
- Latest local evidence: latest app-source full-suite result bundle `/tmp/buysell-submit-readiness-full-8806b12-1784510561.xcresult`, unit result bundle `/tmp/buysell-full-unit-tests-9.xcresult`, focused localization/Today result bundle `/tmp/buysell-localization-today-focused-1784509943.xcresult`, required screenshot result bundles `/tmp/buysell-m10-screenshots-iphone-16-pro-max.xcresult` and `/tmp/buysell-m10-screenshots-ipad-pro-13.xcresult`, local app/unit/UI source typecheck log `/tmp/buysell-submit-readiness-typecheck-8806b12.log`, chunked UI summary `/tmp/buysell-m10-ui-tests/summary.log`, chunked UI evidence log `/tmp/buysell-submit-readiness-ui-8806b12.log`, no-sign archive `/tmp/buysell-submit-readiness-nosign-8806b12.xcarchive`, no-sign verifier log `/tmp/buysell-submit-readiness-nosign-8806b12.log`, performance evidence log `/tmp/buysell-submit-readiness-performance-8806b12.log`, latest-design SDK blocker log `/tmp/buysell-submit-readiness-latest-design-sdk-8806b12.log`, signed-preflight blocker log `/tmp/buysell-submit-readiness-signed-preflight-8806b12.log`, App Store export blocker log `/tmp/buysell-submit-readiness-export-preflight-8806b12.log`, App Store validation blocker log `/tmp/buysell-submit-readiness-app-store-validation-preflight-8806b12.log`, App Store metadata evidence log `/tmp/buysell-submit-readiness-app-store-metadata-8806b12.log`, Supabase deploy-preflight blocker log `/tmp/buysell-submit-readiness-supabase-deploy-8806b12.log`, Supabase schema static check log `/tmp/buysell-submit-readiness-supabase-schema-8806b12.log`, backend blocker log `/tmp/buysell-submit-readiness-backend-8806b12.log`, Supabase function type-check log `/tmp/buysell-submit-readiness-supabase-functions-8806b12.log`, real-device blocker log `/tmp/buysell-submit-readiness-real-device-preflight-8806b12.log`, real-device acceptance evidence blocker log `/tmp/buysell-submit-readiness-acceptance-evidence-8806b12.log`, secret-scan log `/tmp/buysell-submit-readiness-secret-scan-8806b12.log`, Instruments evidence blocker log `/tmp/buysell-submit-readiness-instruments-8806b12.log`, and combined readiness blocker log `/tmp/buysell-submit-readiness-combined-8806b12.log`.
- Default retained artifact paths for the combined gate remain `/tmp/buysell-submit-readiness-full.xcresult`, `/tmp/buysell-local-source-typecheck.log`, `/tmp/buysell-submit-readiness-app-store-metadata.log`, `/tmp/buysell-submit-readiness-today-feature.log`, `/tmp/buysell-submit-readiness-supabase-deploy.log`, `/tmp/buysell-submit-readiness-supabase-schema.log`, `/tmp/buysell-submit-readiness-backend.log`, `/tmp/buysell-submit-readiness-instruments.log`, and `/tmp/buysell-submit-readiness-combined.log`; when using the suffixed evidence above, pass the matching `M10_*` environment variables shown below.
...
The App Store support/privacy/accessibility site source is in `AppStoreSite/`. Sites version 4 is saved from commit `1166ed88e30a28bdb159773010b77d09d555b3b4` and deployed at `https://buysell-ai-support.o5hinwavve.chatgpt.site`; site access is public, and unauthenticated support, privacy, accessibility, and terms requests return `200`. Sites version 6 is saved from support-site commit `72d2908cc8efc2dae13a0f6fa4b86131b3245f81` after raising the support-site build watchdog, but it is not deployed because public deployment still needs explicit approval. The metadata verifier checks that filled-in Support URL, Privacy Policy URL, and Accessibility URL values are public HTTPS pages reachable without authentication. App Store screenshot evidence is retained in `AppStoreAssets/Screenshots/iPhone-16-Pro-Max/` and `AppStoreAssets/Screenshots/iPad-Pro-13-inch-M4/`.
