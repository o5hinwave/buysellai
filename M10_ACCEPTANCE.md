...
The combined gate checks that pass logs retain their concrete artifact markers: no-sign `archive:` and `app size:`, signed `archive:`, `sign in with apple:`, and `release build:`, App Store export `archive:`, `export:`, `ipa:`, `sign in with apple:`, and `release build:`, App Store validation `ipa:`, `sign in with apple:`, and `release build:`, real-device `device:`, `device name:`, `device id:`, `app:`, `sign in with apple:`, and `release build:`, performance `full suite:`, `archive log:`, `performance tests:`, and `app size:`, and Instruments `file:`. It also confirms the App Store validation log references the same `ipa:` path produced by the export preflight, the signed archive, App Store export, App Store validation, and real-device preflight logs retain the same `sign in with apple:` entitlement marker, the same logs record the same `release build:` value, the acceptance `Signed archive` metadata references the signed-preflight `archive:` path, the acceptance `App Store validation` metadata references the validated `ipa:` path, the acceptance and Instruments `Release build` metadata match the signed-preflight `release build:` value, the acceptance and Instruments `Device model` metadata reference the real-device preflight `device name:` value, the Instruments `Signed archive` metadata references the same signed-preflight `archive:` path, and the acceptance and Instruments `iOS version` and `Release build` metadata match for the same physical-device pass. If your retained artifacts use non-default paths, set `M10_NOSIGN_ARCHIVE`, `M10_SIGNED_ARCHIVE`, `M10_APP_STORE_ARCHIVE`, `M10_APP_STORE_EXPORT`, and `M10_INSTRUMENTS_EVIDENCE` before running this gate.
