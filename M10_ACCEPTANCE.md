...
The workflow source is `.github/workflows/app-store-preflight.yml` and covers the signed archive, App Store Connect IPA export, App Store validation preflight, and retained artifact upload from the cloud runner. Use `allow_missing_credentials=true` to keep the workflow as pending evidence while the encrypted Apple credentials are not yet available.
